...
export default class CIFrame extends React.Component {
	...
	componentDidMount() {
		$(this.refs['iframeParent'].querySelectorAll('iframe')[0].contentWindow.document).on('send', (data) => {
			console.log('submitting: ', $(this.refs['iframeParent'].querySelectorAll('iframe')[0].contentWindow.document).find('#gform'))
			$(this.refs['iframeParent'].querySelectorAll('iframe')[0].contentWindow.document).find('#gform').submit();
